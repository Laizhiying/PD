                               ...
                               struct pbuf *p, struct ip_addr *addr, u16_t port)
{
    ...
    if(!strcmp("0x20", (char *)q->payload))//接收到0x20表示开始采样
    {
    	num = 0;
    	Xil_Out32(XPAR_EMC_0_S_AXI_MEM0_BASEADDR+16,0);//将停止采样清零
    	XGpio_DiscreteWrite(&Gpio, 1, 1);//开始采样表示FPGA开始工作指示灯亮
    	udp_printf("sent\r\n",6);
    	printf("sent!\r\n");
    ...
    else if(!strcmp("0x33", (char *)q->payload))//接收到0x33表示停止采样
    {
    	stop_flag = 1;
    	udp_printf("stop\r\n",6);
    	printf("stop!\r\n");
