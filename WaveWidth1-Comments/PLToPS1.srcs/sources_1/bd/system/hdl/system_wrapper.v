...
/********************************/
  ...
  (*mark_debug="true"*)wire [0:0]GPIO_0_tri_o;
  wire [31:0]mem_a_0;
  wire [0:0]mem_cen_0;
  reg  [31:0]mem_dq_i_0;
...
if(!rst_key)begin
  ...
  data_reg7       <= 32'd0;
    ...
    case(mem_a32)
     16'h0:begin
        data_reg1     <=mem_dq_o_0;//MonopulseExThreshould
     end
     16'h1:begin
        data_reg2     <=mem_dq_o_0;//MonopulseExHold
     end     
     16'h2:begin
        data_reg3     <=mem_dq_o_0;//MonopulseMultiThreshould
     end   
     16'h3:begin
        data_reg4     <=mem_dq_o_0;//MonopulseEndHold
     end   
     16'd4:begin
        data_reg5     <=mem_dq_o_0;//StopOrder
     end   
     16'd5:begin
        data_reg6     <=mem_dq_o_0;//GentalThreshould
     end  
     16'd6:begin
        data_reg7     <=mem_dq_o_0;//GentalThreshouldNum
     end                                         
    default : begin
      data_reg1     <= data_reg1;
      data_reg2     <= data_reg2;
...
//检测停止采样信号的上升沿
(*mark_debug="true"*)reg  data_reg5_1;
(*mark_debug="true"*)reg  data_reg5_2;
always @ (posedge FCLK_CLK0_0 or negedge rst_key)
if(!rst_key)begin
  data_reg5_1     <= 0;
    data_reg5_2     <= 0;
...
//GPIO2_0_tri_o对应ARM的XGpio_DiscreteWrite(&Gpio, 2, 0)，表示一个单脉冲处理状态的标志，置1表示arm接收数据结束
wire SendOver_wire;
assign SendOver_wire       = GPIO2_0_tri_o;
//arm接收数据结束，检测开始读取数据的上升沿
(*mark_debug="true"*)reg  SendOver_reg1;
(*mark_debug="true"*)reg  SendOver_reg2;
(*mark_debug="true"*)wire SendOver1;
always @ (posedge FCLK_CLK0_0 or negedge rst_key)
if(!rst_key)begin
    SendOver_reg1     <= 0;
    SendOver_reg2     <= 0;
...
//当FeatureExtraction_flag_reg置1后表示可以发送数据了，当SendOver1置1表示要开始读取新数据，处理新的单脉冲
(*mark_debug="true"*)reg SendOver;
always @ (posedge FCLK_CLK0_0 or negedge rst_key) 
if(!rst_key)begin
  SendOver      <= 0;
end
...
if(!rst_key)begin
  mem_dq_i_0      <= 32'd0;
    ...
    case(mem_a32)
      ...
      16'd12:begin
         mem_dq_i_0   <=Front_Width;
      end 
      16'd13:begin
         mem_dq_i_0   <=Behind_Width;
      end        
      16'd14:begin
         mem_dq_i_0   <=Total_Width;
      end   
      16'd15:begin
...
wire  [15:0]              Front_Width;
wire  [15:0]              Behind_Width;
wire  [15:0]              Total_Width;
wire  [7:0]               Peak_value;
...
(*mark_debug="true"*)wire                      StartSample;
...
//GPIO_0_tri_o对应ARM的XGpio_DiscreteWrite(&Gpio, 1, 0)，表示25个工频周期采样状态
assign StartSample       = GPIO_0_tri_o;
//检测开始采样信号的上升沿
(*mark_debug="true"*)reg  StartSample_reg1;
(*mark_debug="true"*)reg  StartSample_reg2;
(*mark_debug="true"*)wire StartSample1;
always @ (posedge FCLK_CLK0_0 or negedge rst_key)
if(!rst_key)begin
    StartSample_reg1        <= 0;
    StartSample_reg2        <= 0;
